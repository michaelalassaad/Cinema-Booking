...
import { StyleSheet, Text, Image, View, TouchableOpacity, ListViewComponent, Modal } from "react-native";
...
const MovieScreen = ({ navigation }) => {
  ...
  useEffect(async () => {
    const id = getID();
    try {
      const res = await axios.get('http://192.168.0.110:3000/movie/', { params: { movID: id } });
      setMovie(res.data);
      const res1 = await axios.get('http://192.168.0.110:3000/act/', { params: { movID: id } });
      setActor(res1.data);
      const res2 = await axios.get('http://192.168.0.110:3000/dir/', { params: { movID: id } });
      setDirec(res2.data);
      const res3 = await axios.get('http://192.168.0.110:3000/review/', { params: { movID: id } });
      setReview(res3.data);
  ...
  const actors = () => {
    return actor.map(function (actor, i) {
      return (
        <View key={i}>
          <Text style={styles.values}>{actor.firstName} {actor.lastName} </Text>
        </View>
      );
    });
  }
  const directors = () => {
    return direc.map(function (direc, i) {
      return (
        <View key={i}>
          <Text style={styles.values}>{direc.firstName} {direc.lastName} </Text>
        </View>
      );
    });
  }
    ...
    <ScrollView showsVerticalScrollIndicator={false} style={styles.container}>
      ...
      >
        <TouchableOpacity onPress={() => setShowM(false)} style={styles.mod}>
          <Image source={{ uri: movie.poster }}
            style={{
              width: '85%', height: '75%', borderColor: "#1E1F21", borderWidth: 7,
            }} />
        </TouchableOpacity>
      </Modal>
      ...
      <View style={styles.container2}>
        <TouchableOpacity onPress={() => setShowM(true)}>
          <Image source={{ uri: movie.poster }}
            style={{
              width: 150, height: 225, marginVertical: 10, marginLeft: 15,
              borderColor: "#1E1F21", borderWidth: 7, borderRadius: 10
            }} />
        </TouchableOpacity>
      ...
      <Button
        ...
        onPress={() => { navigation.navigate('Login'); }} //to be changed
      />
    </ScrollView >
